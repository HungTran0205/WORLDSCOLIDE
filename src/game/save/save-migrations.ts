...
const migrateV1toV2: MigrationFn = (envelope) => ({
  ...
  gameState: {
    ...envelope.gameState,
    activeMissions: (envelope.gameState.activeMissions as Array<Record<string, unknown>>).map(
      (m) => ({
        ...m,
        phase: 'traveling',
        arrivalTime: null,
...
const migrateV2toV3: MigrationFn = (envelope) => ({
  ...
  gameState: {
    ...envelope.gameState,
    guildHall: {
      ...(envelope.gameState.guildHall as Record<string, unknown>),
      rooms: ((envelope.gameState.guildHall as Record<string, unknown>).rooms as Array<Record<string, unknown>>).map(
 ...
 * Index 0 = v1→v2, Index 1 = v2→v3.
 */
export const migrations: MigrationFn[] = [migrateV1toV2, migrateV2toV3];
