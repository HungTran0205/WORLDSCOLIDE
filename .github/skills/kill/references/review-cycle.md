...
cycle = 0
LOOP:
  1. Run tm.code-reviewer → score, critical_count, warnings, suggestions
...
cycle = 0
LOOP:
  1. Run tm.code-reviewer → score, critical_count, warnings
  ...
  3. ELSE IF critical_count > 0 AND cycle < 3:
     ...
     → Re build/re-run tests to validate fixes
     → cycle++, LOOP
