...
Entry point for using the `tm:ai-multimodal` skill to generate and analyze visual assets that align with frontend design thinking and aesthetic guidelines.

## When to Use

Use `tm:ai-multimodal` in frontend design when you need to:
...
Use `aesthetic` for overall design system guidance and quality evaluation framework. Then use `tm:frontend-design` with `tm:ai-multimodal` for asset generation and analysis that follows those guidelines.

### With `tm:chrome-devtools` Skill
Use `tm:chrome-devtools` to capture full-screen screenshots from inspiration websites for design extraction. Capture at actual viewport size, not full-page scrolls.

### With `tm:ui-styling` Skill
