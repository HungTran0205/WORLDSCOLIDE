...
Complete workflow for generating design-aligned visual assets using `tm:ai-multimodal` skill.
...
Use `tm:ai-multimodal` skill's image generation capabilities:
...
Use `tm:ai-multimodal` to analyze the generated asset (see `visual-analysis.md` for complete workflow):
...
4. Consider using `tm:media-processing` skill for post-generation adjustments
