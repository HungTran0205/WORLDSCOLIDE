---
name: tm:skill-creator
metadata: version: 1.00
description: Guide for creating effective skills, adding skill references, skill scripts or optimizing existing skills. This skill should be used when users want to create a new skill (or update an existing skill) that extends Agent's capabilities with specialized knowledge, workflows, frameworks, libraries or plugins usage, or API and tool integrations.
...
**IMPORTANT:**
- Skills are not documentation, they are practical instructions for Agent to use the tools, packages, plugins or APIs to achieve the tasks.
- Each skill teaches Agent how to perform a specific development task, not what a tool does.
- Agent can activate multiple skills automatically to achieve the user's request.
...
- Descriptions in metadata of `SKILL.md` files should be both concise and still contains enough usecases of the references and scripts, this will help skills can be activated automatically during the implementation process of AI Agent.
...
- **Referenced scripts**:
  ...
  - Make sure scripts respect `.env` file follow this order: `process.env` > `.github/skills/${SKILL}/.env` > `.github/skills/.env` > `.github/.env` 
...
**Why?**
Better **context engineering**: leverage **progressive disclosure** technique of Agent Skills, when agent skills are activated, Agent will consider to load only relevant files into the context, instead of reading all long `SKILL.md` as before.
...
- Make sure scripts respect `.env` file follow this order: `process.env` > `.github/skills/docs-seeker/.env` > `.github/skills/.env` > `.github/.env`
...
**IMPORTANT:**
- Referenced markdown files should be also **less than 100 lines**, remember that you can always split them into multiple files (**progressive disclosure** principle).
- Referenced markdown files are practical instructions for Agent to use the tools, packages, plugins or APIs to achieve the tasks.
- Each skill teaches Agent how to perform a specific development task, not what a tool does.
...
When editing the (newly-generated or existing) skill, remember that the skill is being created for another instance of Agent to use. Focus on including information that would be beneficial and non-obvious to AI Agent. Consider what procedural knowledge, domain-specific details, or reusable assets would help another Agent instance execute these tasks more effectively.
...
3. In practice, how should AI Agent use the skill? All reusable skill contents developed above should be referenced so that Agent knows how to use them.
