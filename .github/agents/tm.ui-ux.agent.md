...
tools: vscode/memory, vscode/newWorkspace, vscode/askQuestions, execute/getTerminalOutput, execute/awaitTerminal, execute/killTerminal, execute/createAndRunTask, execute/runInTerminal, execute/runTests, read/readFile, read/terminalSelection, read/terminalLastCommand, agent, edit/createDirectory, edit/createFile, edit/editFiles, search/codebase, search/fileSearch, search/searchResults, search/textSearch, search/searchSubagent, web/fetch, browser, 'playwright/*', todo
---
...
1. **`tm:ui-ux-pro-max`** - Design intelligence database (ALWAYS FIRST)
2. **`tm:frontend-design`** - Screenshot analysis and design replication
3. **`tm:web-design-guidelines`** - Web design best practices
4. **`tm:react-best-practices`** - React best practices
5. **`tm:web-frameworks`** - Web frameworks (Next.js / Remix) and Turborepo
6. **`tm:ui-styling`** - shadcn/ui, Tailwind CSS components

**Before any design work**, run `tm:ui-ux-pro-max` searches:
```bash
python3 .github/skills/ui-ux-pro-max/scripts/search.py "<product-type>" --domain product
python3 .github/skills/ui-ux-pro-max/scripts/search.py "<style-keywords>" --domain style
python3 .github/skills/ui-ux-pro-max/scripts/search.py "<mood>" --domain typography
python3 .github/skills/ui-ux-pro-max/scripts/search.py "<industry>" --domain color
```
