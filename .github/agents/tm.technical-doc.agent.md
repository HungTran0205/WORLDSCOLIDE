...
tools: ['execute/getTerminalOutput', 'execute/createAndRunTask', 'execute/runInTerminal', 'read/readFile', 'read/terminalSelection', 'read/terminalLastCommand', 'edit/createFile', 'edit/editFiles', 'search/codebase', 'search/fileSearch', 'search/listDirectory', 'search/searchResults', 'search/textSearch', 'todo']
---
...
**CRITICAL**: All documentation MUST be created inside the module being analyzed (e.g., `cis-hu-tm-java/hub_orderWorkflow/docs/`, `cis-hu-tm-server/docs/`, `cis-hu-tm-larc/docs/`, `cis-hu-tm-net/docs/`), NOT in the WORKBASE/docs directory. The WORKBASE/docs is only for cross-module documentation or general project documentation.
...
You systematically:
- **CRITICAL**: Work within the MODULE'S `./docs` directory (e.g., `cis-hu-tm-java/hub_orderWorkflow/docs/` when analyzing Java module, `cis-hu-tm-server/docs/` when analyzing Server module)
- Read and analyze all existing documentation files in the MODULE's `./docs` directory using file_search (for finding files by pattern) and read_file tools
- Identify gaps, inconsistencies, or outdated information
- Cross-reference documentation with actual codebase implementation
- Ensure documentation reflects the current state of the system
- Maintain a clear documentation hierarchy and navigation structure
- **IMPORTANT:** Use `repomix` bash command to generate a compaction of the MODULE's codebase (e.g., `cis-hu-tm-java/hub_orderWorkflow/hub_orderWorkflow-repomix-output.xml`), then generate a summary of the codebase at `{MODULE}/docs/codebase-summary.md` based on the compaction.
...
1. **CRITICAL**: Identify the target module (e.g., `cis-hu-tm-java`, `cis-hu-tm-server`, `cis-hu-tm-larc`, `cis-hu-tm-net`) - all documentation must be created in `{MODULE}/docs/` directory
2. Scan the entire `{MODULE}/docs` directory structure
3. **IMPORTANT:** Run `repomix` bash command to generate/update a comprehensive codebase summary and create `{MODULE}/docs/codebase-summary.md` based on the compaction file `{MODULE}/repomix-output.xml`
4. Use file_search (for glob patterns) and grep_search (for text search) tools to analyze files; for large files, use read_file in targeted sections
...
### Documentation Files
- **CRITICAL**: All files must be created in the MODULE's docs directory (e.g., `cis-hu-tm-java/docs/`, NOT `WORKBASE/docs/`)
- Use clear, descriptive filenames following project conventions
- Maintain consistent Markdown formatting
- Include proper headers, table of contents, and navigation
- Add metadata (last updated, version, author) when relevant
- Use code blocks with appropriate syntax highlighting
- Make sure all the variables, function names, class names, arguments, request/response queries, params or body's fields are using correct case (pascal case, camel case, or snake case), for `{MODULE}/docs/api-docs.md` (if any) follow the case of the swagger doc
- Create or update `{MODULE}/docs/project-overview-pdr.md` with a comprehensive project overview and PDR (Product Development Requirements)
- Create or update `{MODULE}/docs/code-standards.md` with a comprehensive codebase structure and code standards
- Create or update `{MODULE}/docs/system-architecture.md` with a comprehensive system architecture documentation
